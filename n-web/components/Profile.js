function Profile() {
  try {
    ...
    const getFirstRecordDate = () => {
      if (records.length === 0) return '2025年1月';
      const firstRecord = records.sort((a, b) => new Date(a.date) - new Date(b.date))[0];
      return new Date(firstRecord.date).toLocaleDateString('zh-CN');
    };
    ...
    const getTotalDays = () => {
      ...
      const now = new Date();
      return Math.max(1, Math.ceil((now - firstDate) / (1000 * 60 * 60 * 24)));
    };

    return (
      <div className="py-6 space-y-6" data-name="profile" data-file="components/Profile.js">
        <div className="text-center">
          <div className="inline-flex items-center justify-center w-16 h-16 bg-gradient-to-br from-purple-400 to-pink-500 rounded-3xl mb-4">
            <div className="icon-user text-2xl text-white"></div>
          </div>
          <h1 className="text-3xl font-bold bg-gradient-to-r from-gray-900 to-gray-600 bg-clip-text text-transparent">
            个人中心
          </h1>
          <p className="text-gray-500 mt-2">管理你的记录数据</p>
        </div>
